...
from app.ml.models import predict_text_zero_shot, predict_audio_tags
from app.core.utils import parse_filename_fallback, extract_year_from_filename, is_cover_track
...
from app.schemas.analysis import AnalyzeResult, YTMetadata

logger = logging.getLogger("sortund-ai-pipeline")


def format_zero_shot_text_tags(
    raw_text_tags: List[Dict[str, Any]],
) -> List[str]:
...
def parse_youtube_extra_info(yt_extra_info: List[str]) -> Tuple[Optional[str], List[str]]:
...
async def get_base_metadata(
    ...
    if shazam_match:
        ...
        yt_data = await fetch_and_validate_youtube_metadata(artist, title, source_analysis, original_filename)
    else:
...
def get_final_tags(
    text_tags: List[str], audio_tags: List[str], yt_extra_tags: List[str], year: Optional[int]
) -> List[str]:
...
async def execute_analysis_pipeline(temp_file_path: str, original_filename: str) -> AnalyzeResult:
    ...
    # 2. BASE METADATA
    artist, title, source_analysis, yt_data, artwork, isrc = await get_base_metadata(temp_file_path, original_filename)

    # 3. ALBUM
    album_name, yt_extra_tags = parse_youtube_extra_info(yt_data.extra_info)
    ...
    if not final_year:
        ...
        musicbrainz_year_task = asyncio.create_task(fetch_release_year_from_musicbrainz(artist, title))
