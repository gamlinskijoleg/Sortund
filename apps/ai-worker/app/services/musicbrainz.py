        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(f"⚠️ Остаточний збій MusicBrainz ISRC після {max_retries} спроб: {e}")
                    return None
    ...
    try:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"⚠️ Помилка MusicBrainz (спроба {attempt+1}/{max_retries}): {e}. Повтор через 2с...")
                    await asyncio.sleep(2)
                else:
                    logger.warning(f"⚠️ Остаточний збій MusicBrainz після {max_retries} спроб: {e}")
                    return None
